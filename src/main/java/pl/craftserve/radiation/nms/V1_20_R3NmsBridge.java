...
public class V1_20_R3NmsBridge implements RadiationNmsBridge {
    static final Logger logger = Logger.getLogger(V1_20_R3NmsBridge.class.getName());
    ...
    private final Field registryMaterialsUnregisteredIntrusiveHolders;
    ...
    public V1_20_R3NmsBridge(String version) {
        ...
        try {
            ...
            this.mobEffectClass = Class.forName("net.minecraft.world.effect.MobEffect"); // MobEffect ->
                                                                                         // MobEffectInstance
            this.potionRegistryClass = Class.forName("net.minecraft.world.item.alchemy.PotionRegistry"); // PotionRegistry
                                                                                                         // -> Potion
            this.potionBrewerClass = Class.forName("net.minecraft.world.item.alchemy.PotionBrewer"); // PotionBrewer ->
                                                                                                     // PotionBrewing

            Class<?> registryMaterialsClass = Class.forName("net.minecraft.core.RegistryMaterials"); // RegistryMaterials
                                                                                                     // ->
                                                                                                     // MappedRegistry
            this.isRegistryMaterialsFrozen = registryMaterialsClass.getDeclaredField("l"); // l -> frozen
            this.registryMaterialsUnregisteredIntrusiveHolders = registryMaterialsClass.getDeclaredField("m"); // m ->
            ...
            Class<?> minecraftKey = Class.forName("net.minecraft.resources.MinecraftKey"); // MinecraftKey ->
                                                                                           // ResourceLocation
            this.newMinecraftKey = minecraftKey.getMethod("a", String.class); // a -> tryParse
            Class<?> builtInRegistries = Class.forName("net.minecraft.core.registries.BuiltInRegistries"); // RegistryGeneration
                                                                                                           // ->
                                                                                                           // BuiltInRegistries
            this.potionRegistry = builtInRegistries.getDeclaredField("i").get(null); // i -> POTION

            this.getPotion = this.potionRegistry.getClass().getMethod("a", minecraftKey); // a -> get
        } catch (Exception e) {
            throw new RuntimeException("Failed to initialize 1.20.4 bridge", e);
        }
    }
    ...
    public void registerLugolsIodinePotion(NamespacedKey potionKey, LugolsIodinePotion.Config.Recipe config) {
        ...
        try {
            ...
            Object basePotion = this.getPotion.invoke(this.potionRegistry,
                    this.newMinecraftKey.invoke(null, basePotionName));
            ...
            this.registryMaterialsUnregisteredIntrusiveHolders.setAccessible(true);
            this.registryMaterialsUnregisteredIntrusiveHolders.set(this.potionRegistry, new IdentityHashMap());
            this.isRegistryMaterialsFrozen.setAccessible(true);
            this.isRegistryMaterialsFrozen.set(this.potionRegistry, false);
            Object newPotion = this.potionRegistryClass.getConstructor(mobEffectArray.getClass())
                    .newInstance(mobEffectArray);

            Method registerMethod = this.iRegistryClass.getDeclaredMethod("a", this.iRegistryClass, String.class,
                    Object.class); // a -> register
            Object potion = registerMethod.invoke(null, this.potionRegistry, potionKey.getKey(), newPotion);

            Method registerBrewingRecipe = this.potionBrewerClass.getDeclaredMethod("a", this.potionRegistryClass,
                    this.itemClass, this.potionRegistryClass); // a -> addMix
    ...
    public int getMinWorldHeight(World bukkitWorld) {
        ...
        return this.minWorldHeightMap.computeIfAbsent(bukkitWorld.getUID(), worldId -> {
            try {
                ...
                logger.log(Level.SEVERE, "Could not handle min world height on world '" + bukkitWorld.getName() + "' ('"
                        + worldId + "').", e);
                return 0;
            }
        });
